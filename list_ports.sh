...
# Function to find the interface ending with -outside
find_outside_interface() {
    ...
    echo -e "${GREEN}Outside interface found: $interface${TEXTRESET}"
}

# Function to set up nftables rule for SSH on the outside interface
setup_nftables() {
    ...
    # Add a rule to allow SSH on the outside interface, if not already present
